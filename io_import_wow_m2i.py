...
from .bone_mapper_bridge import apply_bone_map_post_import
...
def DoImport(FileName):
	...
	for i in range(0, MeshCount):
		...
		print(
			f' Mesh ID: {Mesh.ID}' +
			f' BM: {Mesh.BlendMode}' +
			f' RF: {Mesh.RenderFlags}'+
			f' TextureTypes: {str(Mesh.TextureTypes)}'+
			f' TextureNames: {str(Mesh.TextureNames)}'+
			f' ShaderID: {Mesh.ShaderId}'+
			f' MaterialOverride: {Mesh.MaterialOverride}'
		)
	...
	for Bone in BoneList: # add bones to armature.
		#print('Instantiated Bone' + str(Bone.Index))
		...
		BEditBone.wow_props.Flags = BoneFlagsToSet(Bone.Flags)
		...
		BEditBone.wow_props.KeyBone = Bone.KeyBone
	...
	for Attachment in AttachmentList:
		...
		if Attachment.Parent >= 0:
			...
			#BAttachment.data.wow_props.ParentBoneID = Attachment.Parent
	...
	for k, Mesh in enumerate(MeshList):
		...
		geoName = '_'+MeshIdToName(meshName)+'_'
		meshName = meshName + geoName
		...
		#print(str(Mesh.RenderFlags) + str(profile_object.data.wow_props.RenderFlags) )
	...
	# -----------------------------------------------------------------------
	m2_name = os.path.splitext(os.path.basename(FileName))[0]
	applied = apply_bone_map_post_import(BArmature, m2_name)
	if applied:
		print(f'[WoW Tools] Bone map "{applied}" auto-applied on import.')
	else:
